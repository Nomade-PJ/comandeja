...
import { DB_CONFIG, getApiUrl } from '@/lib/env';
...
export function DbInfo() {
  ...
  const checkConnection = async () => {
    ...
    try {
      ...
      try {
        const apiUrl = getApiUrl('testConnection');
    ...
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <span>Status da Conexão PostgreSQL</span>
          {connectionStatus && (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
              ...
              <span>{DB_CONFIG.host}</span>
              
              <span className="text-muted-foreground">Porta:</span>
              <span>{DB_CONFIG.port}</span>
              
              <span className="text-muted-foreground">Database:</span>
              <span>{DB_CONFIG.database}</span>
              
              <span className="text-muted-foreground">Usuário:</span>
              <span>{DB_CONFIG.user}</span>
            </div>
          </div>
        </div>
      </CardContent>
